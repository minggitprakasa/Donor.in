<title>Donor.in</title>
<meta http-equiv="X-UA-Compatible" content="IE=edge">
<meta name="viewport" content="width=device-width, initial-scale=1">
<meta charset="UTF-8">

<link href="https://fonts.googleapis.com/css?family=Roboto:300,400,500" rel="stylesheet">

<link href="{{ asset('blog/common-css/bootstrap.css')}}" rel="stylesheet">

<link href="{{ asset('blog/common-css/ionicons.css')}}" rel="stylesheet">

<link href="{{ asset('blog/common-css/swiper.css')}}" rel="stylesheet">

<link href="{{ asset('blog/layout-1/css/styles.css')}}" rel="stylesheet">

<link href="{{ asset('blog/layout-1/css/responsive.css')}}" rel="stylesheet">

<link href="{{ asset('blog/blog-sidebar/css/styles.css')}}" rel="stylesheet">

<link href="{{ asset('blog/blog-sidebar/css/responsive.css')}}" rel="stylesheet">

@section('header-tambahan')

@show

